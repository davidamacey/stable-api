...
from src.utilities import PIPELINE_TYPE, TRT_LOGGER, add_arguments, process_pipeline_args
    ...
    def run(self, prompt, negative_prompt, height, width, batch_size, batch_count, num_warmup_runs, use_cuda_graph, **kwargs_infer_refiner):
        ...
        if num_warmup_runs > 0:
            ...
            for _ in range(num_warmup_runs):
                images, _, _ = self.base.infer(prompt, negative_prompt, height, width, warmup=True)
                if self.enable_refiner:
                    images, _, _ = self.refiner.infer(prompt, negative_prompt, height, width, input_image=images, warmup=True, **kwargs_infer_refiner)
        ...
        for _ in range(batch_count):
            ...
            latents, base64_images, time_base = self.base.infer(prompt, negative_prompt, height, width, warmup=False)
            if self.enable_refiner:
                images, base64_images, time_refiner = self.refiner.infer(prompt, negative_prompt, height, width, input_image=latents, warmup=False, **kwargs_infer_refiner)
                ret.append(images)
            else:
                ret.append(latents)
...
if __name__ == "__main__":
    ...
    ### Inputs required
    # args
    # Namespace(prompt=['Black bear mom carrying cubs across mountain road, 8k'], negative_prompt=[''], batch_size=1, batch_count=1, denoising_steps=30, scheduler=None, lora_scale=None, lora_path=None, onnx_opset=18, onnx_dir='/workspace/stable-diffusion-xl-1.0-tensorrt/sdxl-1.0-base', framework_model_dir='pytorch_model', engine_dir='engine', build_static_batch=True, build_dynamic_shape=False, build_enable_refit=False, build_all_tactics=False, timing_cache=None, use_cuda_graph=True, nvtx_profile=False, torch_inference='', seed=None, output_dir='output', hf_token=None, verbose=False, version='xl-1.0', height=1024, width=1024, num_warmup_runs=1, guidance_scale=5.0, enable_refiner=False, image_strength=0.3, onnx_refiner_dir='/workspace/stable-diffusion-xl-1.0-tensorrt/sdxl-1.0-refiner', engine_refiner_dir='engine_xl_refiner')
    # kwargs_init_pipline
    # {'version': 'xl-1.0', 'max_batch_size': 4, 'denoising_steps': 30, 'scheduler': None, 'guidance_scale': 5.0, 'output_dir': 'output', 'hf_token': None, 'verbose': False, 'nvtx_profile': False, 'use_cuda_graph': True, 'lora_scale': None, 'lora_path': None, 'framework_model_dir': 'pytorch_model', 'torch_inference': ''}
    # kwargs_load_engine
    # {'onnx_opset': 18, 'opt_batch_size': 1, 'opt_image_height': 1024, 'opt_image_width': 1024, 'static_batch': True, 'static_shape': True, 'enable_all_tactics': False, 'enable_refit': False, 'timing_cache': None}
    # args_run_demo
    # (['Black bear mom carrying cubs across mountain road, 8k'], [''], 1024, 1024, 1, 1, 1, True)

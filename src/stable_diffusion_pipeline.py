...
class StableDiffusionPipeline:
    ...
    def save_image(self, images, pipeline, prompt):
        ...
        base64_images = save_image(images, self.output_dir, image_name_prefix)
    ...
    def infer(
        ...
        if not warmup:
            ...
            if not self.return_latents and save_image:
                base64_images = self.save_image(images, self.pipeline_type.name.lower(), prompt)

        return (latents, walltime_ms) if self.return_latents else (images, base64_images, walltime_ms)
